...
public class UnlockCommand implements Command, MessageCreateListener {
    ...
    private Map<Long, Map<Long, Integer>> attempts = new HashMap<>();
            ...
            if (attempts.getOrDefault(server.getId(), new HashMap<>()).containsKey(user.getId())) {
                event.getSlashCommandInteraction().createImmediateResponder()
                        .addEmbed(EmbedTemplate.error()
                                .setDescription("Du hast bereits eine Nummer erhalten, die du eingeben musst!" +
                                ...
                                .addField("English description", "You have already received a number that you need to enter!" +
                ...
                event.getSlashCommandInteraction().createImmediateResponder()
                        .addEmbed(EmbedTemplate.info()
                                .setDescription("Hey " + user.getMentionTag() + ", du hast es fast geschafft! Gebe nun noch `"
                                        + NumberToTextGerman.intToText(number) + "` in Ziffern (0-9) hier in den Chat. Du hast dafür 5 Minuten Zeit.")
                                .addField("English translation", "You almost made it! Now enter `"
                                        + NumberToTextEnglish.intToText(number) + "` in digits (0-9) here in the chat. You have 5 minutes to do this."))
                        .respond().join();
                attempts.computeIfAbsent(server.getId(), (id) -> new HashMap<>())
                        .put(user.getId(), number);
    ...
    public void onMessageCreate(MessageCreateEvent event) {
        if (event.getServer().isPresent()
                && WELCOME_CHANNELS.getOrDefault(event.getServer().get().getId(), 123L)
            ...
            if (attempts.containsKey(serverId) && attempts.get(serverId).containsKey(user.getId())) {
                ...
                try {
                    ...
                    if (sent == answer) {
                        ...
                        Optional<Role> optRole = Database.ROLES.getRole(event.getServer().get().getId(), RoleType.MEMBER);

                        if (optRole.isPresent()) {
                            optRole.get().addUser(user).join();
                            attempts.getOrDefault(serverId, new HashMap<>()).remove(user.getId());
                            event.getMessage().reply(EmbedTemplate.success()
                                    .setDescription(user.getNicknameMentionTag() + " hat sich erfolgreich freigeschaltet.")
                                    .addField("English translation", user.getNicknameMentionTag() + " has successfully unlocked."));
                        } else {
                            LOGGER.error("Die Mitglieds-Rolle ist für den Server " + event.getServer().get().getName() + " nicht gesetzt!");
                            event.getMessage().reply(EmbedTemplate.error()
                                    .setDescription("Es ist keine Mitglieds-Rolle für diesen Server gesetzt. Dies ist ein Fehler der Administration."));
                    ...
                    event.getMessage().reply(EmbedTemplate.info()
                            .setDescription(noticeGerman)
                            .addField("English translation", noticeEnglish)).join();
                } catch (NumberFormatException e) {
                    event.getMessage().reply(EmbedTemplate.error()
                            .setDescription("Bitte gebe die dir gesendete Nummer in Ziffern (0-9) ein: `"
                                    + NumberToTextGerman.intToText(answer)
                                    + "`")
                            .addField("English translation", "Please enter the number sent to you in digits (0-9): `"
    ...
    public static void reloadWelcomeChannels() {
        ...
        DiscordBot.INSTANCE.api.getServers().forEach(server -> {
            Database.CHANNELS.getChannel(server.getId(), ChannelStore.ChannelType.WELCOME_CHANNEL).ifPresent(channel -> {
    ...
    private String getNoticeString(String message, int sent, int answer, boolean english) {
        ...
        if (answerLength == message.length()) {
            ...
            if (difference.equals(answerStr)) {
                ...
                return (english ? "Note: " : "Hinweis: ") + StringUtils.reverse(reverseAnswerStr.replaceFirst(differenceReversed,
                        differenceReversed.toUpperCase()));
            }

        } else {
            return english ? "The number is " + answerLength + " digits long." : "Die Zahl ist " + answerLength + " Ziffern lang.";
        }
    }
}
