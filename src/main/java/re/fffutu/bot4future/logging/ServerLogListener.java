...
public class ServerLogListener implements ServerThreadChannelCreateListener, ServerThreadChannelDeleteListener, ServerThreadChannelUpdateListener {

    @Override
    public void onThreadCreate(ThreadCreateEvent event) {
        if (!ServerStore.isInSet(event.getServer().getId(), ServerStore.LOGGED_THREADS, event.getChannel().getIdAsString())) {
            ServerStore.addToSet(event.getServer().getId(), ServerStore.LOGGED_THREADS, event.getChannel().getIdAsString());
            ChannelStore.getChannel(event.getServer().getId(), ChannelStore.ChannelType.SERVER_LOG).ifPresent(channel -> {
                ...
                log.getEntries().stream().filter(entry -> entry.getType() == AuditLogActionType.THREAD_CREATE
                    ...
                    channel.asTextChannel().get().sendMessage(EmbedTemplate.success()
                            ...
                            .addField("Thread-Name", thread.getMentionTag(), true)
                            .addField("Thread-ID", thread.getId() + "", true)
                            .addField("Ersteller", creator.getMentionTag() + " (" + creator.getId() + ")", false)
                            .addField("Channel", thread.getParent().getMentionTag(), true)
                            .addField("Channel-ID", thread.getParent().getId() + "", true)
                            .addField("Erstellt", "<t:" + thread.getCreationTimestamp().getEpochSecond() + ":R>")
                            .addField("Privater Thread", thread.isPrivate() ? "Ja" : "Nein")
                            .addField("Archiviert in", thread.getAutoArchiveDuration() + " Minuten")
        ...
        ChannelStore.getChannel(event.getServer().getId(), ChannelStore.ChannelType.SERVER_LOG).ifPresent(channel -> {
            ...
            log.getEntries().stream().filter(entry -> entry.getType() == AuditLogActionType.THREAD_DELETE
                ...
                channel.asTextChannel().get().sendMessage(EmbedTemplate.error()
                        ...
                        .addField("Channel", thread.getParent().getMentionTag(), true)
                        .addField("Channel-ID", thread.getParent().getId() + "", true)
                        .addField("Thread-Status", "Archiviert: " + (thread.isArchived() ? "Ja" : "Nein")
                                + "\nGesperrt: " + (thread.isLocked() ? "Ja" : "Nein")
                                + "\nPrivat: " + (thread.isPrivate() ? "Ja" : "Nein"))
                        .addField("Mitglieder", thread.getMemberCount() + "")
                        .addField("Nachrichten", thread.getMessageCount() + "")
                        .addField("Löschender User", deleter.getMentionTag() + " (" + deleter.getId() + ")")
                        .addField("Erstellt", "<t:" + thread.getCreationTimestamp().getEpochSecond() + ":R>")
                        .addField("Gelöscht", "<t:" + entry.getCreationTimestamp().getEpochSecond() + ":R>")
